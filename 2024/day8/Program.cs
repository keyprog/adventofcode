...
string[] map = File.ReadAllLines("/home/alexp/dev/adventofcode/2024/day8/input.txt");
...
foreach((char a, (int Row, int Col)[] coors) in antennaes)
{
    ...
    for(int i = 0; i<coors.Length-1;++i)
        for(int j = i+1; j<coors.Length; ++j)
        {
            ...
            antinodes.Add(coors[i].Row - dr, coors[i].Col - dc);
            antinodes.Add(coors[j].Row + dr, coors[j].Col + dc);
        }
    }
}

antinodes.Antinodes.PrintCollection();
