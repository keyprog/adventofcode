...
Graph.ToJson(
    conf.Modules.Values.Select(m => new Graph.Node(m.Name, m is FlipFlopModule ? "green" : "red")),
    conf.Modules.Values.SelectMany(m => m.Outputs.Select(o => new Graph.Edge(m.Name, o)))
).Print();

//for(int i=0;i<100;++i)
//  conf.PushButton();
while (!conf.PushButton(count) && count < 100000)
{
    count++;
    //if (count % 100000/ == 0) count.Print();
}
count.Print();
...
class ModulesConfiguration
{
    ...
    public bool PushButton(long step)
    {
        int low = 0;
        int high = 0;
        ...
        while (pulses.Count > 0)
        {
            ...
            if (pulse.Source is "th" && pulse.isHigh)
                sb.Append($"{pulse.Destination}:{(pulse.isHigh ? 1 : 0)};");
