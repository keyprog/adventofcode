...
decimal minX = t1.Where(t => t.VX < 0).Select(t => t.X).Min().Print();
decimal maxX = t1.Where(t => t.VX > 0).Select(t => t.X).Max().Print();

decimal minY = t1.Where(t => t.VY < 0).Select(t => t.Y).Min().Print();
decimal maxY = t1.Where(t => t.VY > 0).Select(t => t.Y).Max().Print();

decimal minZ = t1.Where(t => t.VZ < 0).Select(t => t.Z).Min().Print();
decimal maxZ = t1.Where(t => t.VZ > 0).Select(t => t.Z).Max().Print();
return;
Stone[] t2 = cloud.Stones[0..3].Select(s => s.Move(1)).ToArray();
int i = 1;
...
//for (long i = 2; i < 100000; i++)
{
    ...
    if (i% 10000000 == 0)
        Console.WriteLine(i);
    t2 = t2.Select(s => s.Move(1)).ToArray();
