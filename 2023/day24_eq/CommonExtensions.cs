...
public static class Graph
{
    ...
    public static string ToJson(IEnumerable<Node> nodes, IEnumerable<Edge> edges)    
        => JsonSerializer.Serialize(new GraphVis(nodes.OrderBy(n => n.Label).ToArray(), edges.OrderBy(e => e.From).ThenBy(e => e.To).ToArray()), new JsonSerializerOptions{ PropertyNamingPolicy = JsonNamingPolicy.CamelCase });    
    ...
    public record Edge(string From, string To, string? Label = null);
}